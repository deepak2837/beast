...
import { sendEmail } from "../Client/emailClient.js";
...
export const ticketCreater = async (req, res) => {
  ...
  try {
    ...
    if (tickets) {
      ...
      sendEmail( ticket.title,ticket.description,ticket._id,user.userId,`${user.email}`+","+`${engineer.email}`)
    }
    res.send(ticket);
  } catch (err) {
...
export const ticketUpdate = async (req, res) => {
  ...
  if (user.userType === constants.userTypes.customer) {
    ...
    if (owns_ticket(req.params.ticketId, user)) {
      ...
      ticket.save();
      sendEmail( `${ticket.title} has been updated `,`${ticket.description} has been updated `,ticket._id,user.userId,`${user.email}`+","+`${engineer.email}`)
    ...
    if (owns_ticket_assignment(req.params.ticketId, user)) {
      console.log("going inside enginner ");
      console.log(ticket);
      ...
      ticket.save();
      const this_user =  await User.findOne({ ticketCreated: { $in: [req.params.ticketId] } });
      sendEmail( "ticket status has been updated",`tick status is now ${ticket.status}  `,ticket._id,user.userId,`${this_user.email}`+","+`${engineer.email}`)
      res.send(ticket).status(200);
    } else {
    ...
    ticket.ticketPriority = req.body.priority
      ...
      : (ticket.ticketPriority = ticket.ticketPriority);
    if (
      req.body.status === constants.ticketstatus.closed ||
      req.body.status === constants.ticketstatus.inprogress ||
      req.body.status === constants.ticketstatus.open ||
      req.body.status === constants.ticketstatus.blocked
    ) {
      ticket.status = req.body.status;
    }
    ...
    ticket.save();
    const this_user =  await User.findOne({ ticketCreated: { $in: [req.params.ticketId] } });
    const this_engineer =  await User.findOne({ tickeAssigned: { $in: [req.params.ticketId] } });
      sendEmail( "ticket status has been updated",`tick status is now ${ticket.status}  `,ticket._id,user.userId,`${this_user.email}`+","+`${user.email}`+","+`${this_engineer.email}`)
...
export const getticketbyid = async (req, res) => {
  ...
  if (
    ...
    ticket.assignee === req.body.userId ||
    ticket.reporter === req.body.userId
  ) {
